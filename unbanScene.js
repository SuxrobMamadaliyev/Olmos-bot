...
const unbanScene = new Scenes.BaseScene('unbanScene');

unbanScene.enter(async (ctx) => {
  await ctx.reply(
    `✅ Blokdan chiqarish uchun foydalanuvchining Telegram ID raqamini yuboring:`,
    Markup.keyboard([['❌ Bekor qilish']]).resize()
  );
});

unbanScene.hears('❌ Bekor qilish', async (ctx) => {
  await ctx.reply('Bekor qilindi.', { reply_markup: { remove_keyboard: true } });
  return ctx.scene.leave();
});

unbanScene.on('text', async (ctx) => {
  ...
  if (!user.isBlocked) {
    await ctx.reply(`⚠️ Foydalanuvchi bloklanmagan: @${user.username || 'nomaʼlum'} (${telegramId})`, {
      reply_markup: { remove_keyboard: true },
    });
  } else {
    user.isBlocked = false;
    await user.save();
    await ctx.reply(`✅ Foydalanuvchi blokdan chiqarildi: @${user.username || 'nomaʼlum'} (${telegramId})`, {
      reply_markup: { remove_keyboard: true },
    });
  }
